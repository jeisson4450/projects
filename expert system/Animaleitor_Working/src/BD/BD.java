...
//    public boolean escribir_experto(int IDP, int ID, String PREMISA, String DESCRIPCION) {
    ...
    public boolean actualizar_expertoID(long IdAnterior, long IdNuevo){
        try {
            conectar();  //mod fredy
            String sql = "UPDATE SISTEMA_EXPERTO SET ID="+IdNuevo+" WHERE ID="+IdAnterior;
            Statement sentencia;
            sentencia = getConexion().createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
            sentencia.executeUpdate(sql);
            getConexion().commit();
...
//            throws SQLException {
    ...
    public ArrayList<ModAdmin> consultar_sugerenciaLista() {
        ModAdmin sugerencia = new ModAdmin();
        ArrayList<ModAdmin> listaSug = new ArrayList<>();
        try {
            conectar();  //mod fredy
...
//            throws SQLException {
            ...
            while (rs.next()) {
                ...
                String h = rs.getString("RUTA");
                System.out.println(s + " " + f + " " + g + " " + h);
                sugerencia = new ModAdmin(s, f, g, h);
                listaSug.add(sugerencia);
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.out.print("ERROR SQL");
